...
for line in lines:
    ...
    if username in dateline_by_user[dateline].keys():
...
for line in lines:
    if line[keys['uid']] == '573638104' :tweets_total_from_573638104 += 1
...
lines_by_rt = [(line[keys['uid']],int

(line[keys['rt_num']])) for line in lines if line[ keys['rt_num']] != '']
...
uid_by_max_tweets_on_hour11 = d[0][0] # todo 如果有多个最多怎么办？
...
uid_by_max_v_url = uid_sort_by_v_url[0][0] # todo 如果有多个最多怎么办？
